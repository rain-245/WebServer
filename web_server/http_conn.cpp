...
http_conn::HTTP_CODE http_conn::parse_headers(char *text){
    ...
    /*遇到空行,表示头部字段解析完毕*/
    if(text[0] == '\0'){
        /*如果HTTP请求有消息体,则还需要读取m_content_length字节的消息,状态机转移到CHECK_STATE_CONTENT状态*/
        if(m_content_length != 0){
            m_check_state = CHECK_STATE_CONTENT;
            return NO_REQUEST;
        }
        /*否则说明我们以及得到了一个完整的HTTP请求*/
        return GET_REQUEST;
    }
...
/*我们没有真正解析HTTP请求的请求体,只是判断它是否被完整地读入了*/
...
bool http_conn::write(){
    ...
    while(1){
        ...
        if(temp < 0){
            ...
            虽然在此期间,服务器无法立即接受到同一客户的下一个请求,但这可以保证连接的完整性*/
            if(errno == EAGAIN){
...
bool http_conn::add_status_line(int status,const char *title){
    return add_response("%s %d %s/r/n","HTTP/1.1",status,title);
}
...
bool http_conn::add_headers(int content_length){
 ...
 bool http_conn::add_linger(){
    return add_response("Connction: %s\r\n",m_linger == true ? "keep-alive" : "close");
 }
